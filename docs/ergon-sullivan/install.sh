...
# 사용법: curl -fsSL https://genobj.github.io/download-center/ergon-sullivan/install.sh | sudo bash
# 또는:  curl -fsSL https://genobj.github.io/download-center/ergon-sullivan/install.sh | sudo bash -s -- beta
...
if [[ "$DOWNLOAD_URL" == *.deb ]]; then
  ...
  # 바이너리
  curl -L -o "$INSTALL_DIR/slv" "$DOWNLOAD_URL"
  chmod +x "$INSTALL_DIR/slv"
fi
